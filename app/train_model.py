...
def use_existing_model():
    ...
    if not use_nn:
        ...
        create_mesh(y_pred_full.reshape(100, 100), images_base_dir + id_name +
                    prediction_name, title_name="predicted_full_grid_with_svm")
    else:
...
def optimise(pipeline, X_train, y_train):
    ...
    gs = GridSearchCV(pipeline, param_grid=param_grid, scoring="mean_absolute_error",
                      cv=ShuffleSplit(len(X_train), test_size=0.33, n_iter=1, random_state=0))
