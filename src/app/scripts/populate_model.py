...
def step4and5(cursor, dayoftheweek, hour, short_date, avg_co):
    """
        Implement Step 4 and Step 5 of the plan - get all the average co values,
        filtering out the specific co value
    """
    ...
    sql_str = """select datetime, weekdays, dayoftheweek ,co, avg_co from FixedSamples where dayoftheweek="{0}" and time="{1}"and date!="{2}" group by datetime order by ABS(avg_co-{3}) asc, ABS(datediff("{2}",date)) asc;""".format(
        dayoftheweek, hour, short_date, avg_co)
    # start date
...
def get_estimates_value(cursor, day_to_closest, location):
    ...
    for k, val in day_to_closest.items():
        ...
        for row in val:
            try:
                sql_str = """select co from Estimates where grid_location={0} and datetime="{1}";""".format(location, row[
                                                                                                            0])
                cursor.execute(sql_str)
...
def populate_model_table(cursor, input_datetime, grid_location, day_to_co, co):
    ...
    data += ["{0}".format(co)]
    # print data
    # TODO should check the ordering so that the data per day is correctly
    # inserted into the table in the correct order
...
def main():
    ...
    for _ in range(epochs):
        ...
        sql_str = """select datetime, grid_location, co from Estimates where datetime="{0}" order by grid_location asc;""".format(
            first_date)
        cursor.execute(sql_str)
        results = cursor.fetchall()
        ...
        for row in results:
            ...
            sql_str = """select avg_co, datetime from FixedSamples where date="{0}" and time="{1}" order by date asc limit 1;""".format(
                short_date, hour)
            # print sql_str
            # start date
            cursor.execute(sql_str)
            ...
            assert result_datetime.strftime(
                "%Y-%m-%d %H:%M:%S") == first_date.strftime("%Y-%m-%d %H:%M:%S")
            # print "avg_co is ", avg_co

            # Step 4 and 5 - map day to closest records in FixedSamples
            day_to_closest = {}
            (valid, day_to_closest[dayoftheweek]) = step4and5(
                cursor, dayoftheweek, hour, short_date, avg_co)
